...
import { PokemonLevel } from "@/components/pokemon/PokemonLevel";
import { PokemonRelease } from "@/components/pokemon/PokemonRelease";
...
export default function Pokemon() {
  ...
  const onPageScrollStateChanged = (e: {
    ...
    if (offset.current === -1 && id === totalPokemons) {
      return;
    }
    if (offset.current !== 0) {
...
function PokemonView({ id, totalPokemons, onPrevious, onNext }: Props) {
  ...
  return (
    <RootView style={{ backgroundColor: colorType }}>
      <View>
        ...
        <PokemonRelease index={pokemon.id} name={pokemon.name}/>
      </View>
    </RootView>
  );
}
